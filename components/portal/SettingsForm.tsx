...
export default function SettingsForm({ user }: { user: User }) {
  ...
  const saveProfile = async (e: React.FormEvent) => {
    ...
    setProfileStatus(res.ok ? "ok" : "err");
  ...
  const changePassword = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/portal/settings/password", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ current: passwords.current, next: passwords.next }) });
    const data = await res.json();
    if (!res.ok) { setPassError(data.error || "Eroare."); setPassStatus("err"); return; }
    setPassStatus("ok");
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={saveProfile} className="rounded-xl p-6 space-y-4" style={{ background: "#fff", border: "1px solid var(--color-border-warm)" }}>
        ...
        <div className="flex items-center gap-3">
          <button type="submit" disabled={profileStatus === "loading"} className="px-6 py-2.5 rounded-xl text-sm font-semibold text-white flex items-center gap-2 transition-all hover:opacity-90 disabled:opacity-60" style={{ background: "var(--color-gold)" }}>
          ...
          {profileStatus === "err" && <span className="flex items-center gap-1 text-sm" style={{ color: "#EF4444" }}><AlertCircle size={14} /> Eroare.</span>}
        </div>
      </form>
